...
parser.add_argument('--openai-API-key', default=None, type=str,
        help='Get an openai API key for free. See https://beta.openai.com/account/api-keys')
...
class Model:
    ...
    def __init__(self, args):
        ...
        print('\n\tLoading CLIP ViT-L/14 as the VLM...')
    ...
    def load_openimage_classnames(self, csv_path):
        csv_data = f'{csv_path}'
        csv_reader = open(csv_data, encoding="utf-8")
        classnames = {idx: row[-1] for idx, row in enumerate(csv_reader)}
        return classnames

    # Load Tencent ML Images and compute their CLIP feature
